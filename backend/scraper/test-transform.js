...
const testSlugs = [
  ...
  'kukla', 'tutsak-2', 'hizli-ve-ofkeli-10', 'zirve-tutkusu',
];
...
(async () => {
  for (const slug of testSlugs) {
    ...
    try {
      ...
      console.log(`${slug}: ${working.length} working URLs`);
      working.forEach(u => console.log(`  ${u}`));
      await new Promise(r => setTimeout(r, 1000));
    } catch(e) {
      console.log(`${slug}: ERROR ${e.message.substring(0,40)}`);
    }
  }
})();
