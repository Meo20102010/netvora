    ...
    if (!slugs.has(slug)) {
      try {
        await prisma.category.create({
          data: { name: genre.charAt(0).toUpperCase() + genre.slice(1), slug, sortOrder: 0 }
        });
...
async function scrapePage(page) {
  ...
  $('li.film').each((i, el) => {
    ...
    movies.push({ url: link, turkishTitle, originalTitle, year, imdb, quality, genres, poster, page });
  });
...
async function main() {
  ...
  for (let page = lastPage + 1; page <= MAX_PAGES; page++) {
    try {
        ...
        try {
          ...
          await prisma.content.create({
            data: {
              ...
              slug,
              description: `${m.turkishTitle}${m.originalTitle !== m.turkishTitle ? ' (' + m.originalTitle + ')' : ''} filmini full HD kalitesinde izleyin.`,
              type: 'MOVIE',
              posterUrl: m.poster,
              year: m.year,
              imdbRating: m.imdb,
              ...
              language: 'tr',
              isActive: true,
              categoryId: mapCategory(m.genres, categories),
            }
          });
          ...
          errors++;
          errorLog.push({ url: m.url, error: e.message, time: new Date().toISOString() });
        }
      }
      ...
      state.errors = errors;
      if (page % 10 === 0 || errors > 0) {
        saveJSON(STATE_FILE, state);
        saveJSON(ERRORS_FILE, errorLog);
      }

      const rate = pageAdded > 0 ? `+${pageAdded}` : '0';
      console.log(`Page ${page}/${MAX_PAGES}: ${rate} | Total: ${created} created, ${errors} errors`);
      ...
      saveJSON(STATE_FILE, state);
      saveJSON(ERRORS_FILE, errorLog);
      console.error(`Page ${page} error: ${err.message}, retrying after delay`);
      await sleep(3000);
      page--;
    }
  }
