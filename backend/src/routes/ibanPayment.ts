...
export function ibanPaymentRoutes(prisma: PrismaClient): Router {
  ...
  router.post('/:id/receipt', authenticate, upload.single('receipt'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const userId = req.user!.userId;
    const { id } = req.params;
    ...
    const updated = await prisma.ibanPayment.update({
      ...
      data: {
        status: 'RECEIVED',
        receiptUrl: `/storage/receipts/${req.file.filename}`,
