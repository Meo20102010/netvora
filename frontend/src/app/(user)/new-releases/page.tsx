...
export default function NewReleasesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await contentApi.getAll({ sortBy: 'createdAt', limit: 50, page });
        if (res.data.success) {
    ...
    <div className="min-h-screen bg-[#141414]">
      ...
      <main className="pt-24 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <HiSparkles className="w-7 h-7 text-[#E50914]" />
          <h1 className="text-2xl md:text-3xl font-black">Yeni Eklenenler</h1>
        </div>
        {loading ? (
          ...
          <div className="text-center py-20">
            <HiSparkles className="w-16 h-16 text-[#333] mx-auto mb-4" />
            <p className="text-[#555]">Henüz yeni bir içerik eklenmedi.</p>
          </div>
        )}
      </main>
    </div>
