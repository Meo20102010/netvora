...
import { HiFilm, HiFunnel, HiChevronLeft, HiChevronRight } from 'react-icons/hi2';
...
export default function MoviesPage() {
  ...
  useEffect(() => {
    ...
    load();
  }, [isAuthenticated, router, sortBy, page]);

  const load = async () => {
    setLoading(true);
    try {
      const res = await contentApi.getAll({ type: 'MOVIE', sortBy, limit: 50, page });
      if (res.data.success) {
      ...
      <main className="pt-24 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <HiFilm className="w-7 h-7 text-[#E50914]" />
            <h1 className="text-2xl md:text-3xl font-black">Filmler</h1>
          </div>
          <div className="flex items-center gap-2">
            <HiFunnel className="w-4 h-4 text-[#808080]" />
        ...
        {loading ? (
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 mt-8">
                ...
                <span className="text-sm text-gray-400">Sayfa {page} / {totalPages} ({totalItems} film)</span>
          ...
          <div className="text-center py-20">
            <HiFilm className="w-16 h-16 text-[#333] mx-auto mb-4" />
            <p className="text-[#555]">Henüz film bulunamadı.</p>
          </div>
        )}
      </main>
    </div>
