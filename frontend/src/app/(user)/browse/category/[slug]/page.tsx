...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    load();
  }, [isAuthenticated, slug, sortBy, page]);
  ...
  const load = async () => {
    ...
    try {
      ...
      const catsRes = await contentApi.getCategories();
      const cats = catsRes.data.data || [];
      const cat = cats.find((c: any) => c.slug === slug);
      if (cat) {
        setCategoryName(cat.name);
        const res = await contentApi.getAll({ categoryId: cat.id, sortBy, limit: 50, page });
        if (res.data.success) {
      ...
      <main className="pt-24 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl md:text-3xl font-black">{categoryName || slug}</h1>
        ...
        {loading ? (
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 mt-8">
                ...
                <span className="text-sm text-gray-400">Sayfa {page} / {totalPages} ({totalItems} icerik)</span>
          ...
          <div className="text-center py-20">
            <p className="text-[#555]">Bu kategoride henuz icerik bulunmuyor.</p>
          </div>
        )}
      </main>
    </div>
