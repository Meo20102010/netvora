...
import { HiTv, HiFunnel, HiChevronLeft, HiChevronRight } from 'react-icons/hi2';
...
export default function SeriesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await contentApi.getAll({ type: 'SERIES', sortBy, limit: 50, page });
      if (res.data.success) {
      ...
      <main className="pt-24 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <HiTv className="w-7 h-7 text-[#E50914]" />
            <h1 className="text-2xl md:text-3xl font-black">Diziler</h1>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 mt-8">
                ...
                <span className="text-sm text-gray-400">Sayfa {page} / {totalPages} ({totalItems} dizi)</span>
          ...
          <div className="text-center py-20">
            <HiTv className="w-16 h-16 text-[#333] mx-auto mb-4" />
            <p className="text-[#555]">Henüz dizi bulunamadı.</p>
          </div>
        )}
      </main>
    </div>
