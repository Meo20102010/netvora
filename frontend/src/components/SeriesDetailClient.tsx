...
import {
  ...
  Tabs, Badge, GradientButton,
} from '@/components/ui';
import {
  HiPlay, HiPlus, HiCheck, HiHeart, HiOutlineHeart,
  HiStar, HiClock, HiTag, HiArrowLeft,
  HiEye, HiInformationCircle, HiChatBubbleLeftEllipsis,
  HiSparkles,
...
export default function SeriesDetailClient({ content, id, initialSeason, initialEpisode }: SeriesDetailClientProps) {
  ...
  const [activeTab, setActiveTab] = useState(() => initialEpisode ? 'episodes' : 'info');
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [recRes, subRes] = await Promise.allSettled([
